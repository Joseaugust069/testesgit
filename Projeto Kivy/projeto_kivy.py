...
class Menu (Screen):
    ...
    def confirmacao(self,*args, **kwars):
        ...
        animText.start(sim)
        anim = Animation(size=(300,180),duration=1, t= 'out_back')
        anim.start(pop)
...
class Assistencia(Screen):
    assistencia = []
    texto = 'Numero de contato'
    path = ' '
    def on_pre_enter(self):
        self.path = App.get_running_app().user_data_dir+'/'
        self.loadData()
        Window.bind(on_keyboard= self.voltar)
        for assistencia in self.assistencia:
            self.ids.box.add_widget(Assistentes(text=assistencia))
